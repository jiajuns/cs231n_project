...
def load_features(model, num_videos, num_frames, video_idx, labels, size = (224, 224, 3), train_test_flag='train'):
    ...
    if train_test_flag == 'train':
        batch_size = 70 # for completely division
    else:
        batch_size = 30
    ...
    for i, vid in enumerate(video_idx):
        ...
        video_info_list.append((video_path, labels[i], num_frames, h, w, vid))
    ...
    # save data
    np.save(os.getcwd() + '/datasets/X' + train_test_flag + '_allCap' + str(num_frames)\
                 + 'frames.npy', Xtrain)
    np.save(os.getcwd() + '/datasets/y' + train_test_flag + '_allCap'  + str(num_frames) \
                + 'frames.npy', ytrain)
    np.save(os.getcwd() + '/datasets/videoId' + train_test_flag + '_allCap' + str(num_frames)\
                 + 'frames.npy', vid_ls)
...
def process_video(video_info):
    ...
    del vidcap
    return (video_cateogory, video_selected_frames, vid)
